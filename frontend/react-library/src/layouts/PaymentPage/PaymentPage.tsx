...
export const PaymentPage = () => {
        ...
        const fetchFees = async () => {
            if (authState && authState.isAuthenticated) {
                ...
                if (!paymentResponse.ok) {
                    throw new Error("Fetching fees of GET request failed");
                }
    ...
    async function checkout() {
        ...
        if (!stripeResponse.ok) {
            ...
            throw new Error("Something went wrong during payment POST request");
        }
        ...
        stripe.confirmCardPayment(
            ...
            if (result.error) {
                ...
                if (!stripeResponse.ok) {
                    setHttpError(true);
                    setSubmitDisabled(false);
                    throw new Error("Failed to remove fees from account during PUT request");
                }
                setFees(0);
                setSubmitDisabled(false);
            }
