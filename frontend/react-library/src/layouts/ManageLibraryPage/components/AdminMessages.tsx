...
export const AdminMessages = () => {
        ...
        const fetchUserMessages = async () => {
            if (authState && authState.isAuthenticated) {
                ...
                if (!messagesResponse.ok) {
                    throw new Error('Failed GET request. Could not fetch a list of open messages awaiting for a response from the admin.');
                }
    ...
    async function submitResponseToQuestion(id: number, response: string) {
        const url = `${process.env.REACT_APP_API}/messages/secure/admin/messages`;
        ...
        if (authState && authState?.isAuthenticated && id !== null && response !== "") {
            ...
            if (!messageAdminRequestModelResponse.ok) {
                throw new Error("Failed to submit admin response during PUT request.");
            }
